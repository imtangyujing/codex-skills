...
description: |
  ...
  当用户想要系统性研究一个产品、公司、概念、技术或人物时使用。核心是双轴分析：纵轴追踪从诞生到当下的完整生命历程（以叙事故事呈现），横轴在当下时间截面上与竞品/同类进行系统性横向对比，最后交叉两条轴产出独到洞察。最终产出一份结构完整、可直接阅读和继续编辑的Markdown研究报告。
...
你正在执行一次横纵分析法深度研究。最终产出一份**结构完整、可直接阅读和继续编辑的Markdown研究报告**。

## 前置准备

### 环境准备

本Skill已内置完整的写作风格指南（见下文"写作风格"部分），无需额外加载其他skill。
...
Markdown文件命名为 `[研究对象名称]_横纵分析报告.md`，保存到用户的工作目录中。

默认只交付一个Markdown文件，不同时生成HTML、PDF或其他衍生文件。仅在用户明确要求时提供其他格式。

如用户明确要求PDF，可使用本Skill自带的 `scripts/md_to_pdf.py` 转换。转换过程中产生的HTML只作为临时产物，不主动交付。
...
- [ ] 所有关键事实标注了信息来源？
- [ ] 搜不到的信息诚实标注了「暂缺」，没有编造？
- [ ] Markdown结构清晰，标题层级、表格、引用、来源链接均可正常阅读？
